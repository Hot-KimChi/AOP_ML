    ...
    try:
        ...
        def func_sql_get(server_address, ID, password, database, command):
            try:
                ...
                #     selected_DBtable = combo_DBtable.get()
                #     query = f'''
                #     SELECT * FROM {selected_DBtable}
                #     where probeId = {selected_probeId}
                #     order by 1
                #     '''
        ...
        label_probename = Label(root_gen, text='Probe Name')
        label_probename.place(x=10, y=50)

        combo_probename = ttk.Combobox(root_gen, value=list_probe, height=0, state='readonly')
        combo_probename.place(x=120, y=50)
