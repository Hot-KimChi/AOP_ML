...
class ParamUpdate:
    ...
    def param_merge(self):
        ...
        # 중복 열 제거
        cols_to_drop = ['SysTxFreqIndex', 'TxpgWaveformStyle', 'ProbeNumTxCycles', 'IsTxChannelModulationEn', 'IsPresetCpaEn', 'ElevAperIndex']
        df_total_BM = df_total_BM.drop_duplicates(subset=cols_to_drop, keep='first')

        # NumTxElements가 GroupIndex에 따라 전체 데이터가 동일할 경우에만 중복 열 제거
        # NumTxElements가 GroupIndex에 따라 전체 데이터가 동일할 경우 중복된 행 중 첫 번째 행만 유지
        df_total_BM = df_total_BM.drop_duplicates(subset=['NumTxElements', 'GroupIndex'], keep='first')
        # grouped = df_total_BM.groupby('GroupIndex')['NumTxElements'].nunique().reset_index()
        # duplicate_groups = grouped[grouped['NumTxElements'] == 1]['GroupIndex'].tolist()
        # df_total = df_total_BM[~df_total_BM['GroupIndex'].isin(duplicate_groups)]
