...
def show_table(df, database, selected_DBtable, extra):
    n_root = Tk()
    ...
    if len(extra.index) > 1:
        ...
        my_tree_extra = ttk.Treeview(n_root)
...
def measset_gen(server_address, ID, password, database):
    try:
        root_gen = Tk()
        ...
        def insert_data():
            try:
                ...
                print(select_data)
                show_table(df=select_data, database=combo_login.get(), selected_DBtable='Summary: B & M', extra=df.reindex(idx))
                ...
                idx = [x[0] for x in df_dic.values() if len(x) == 1]
                show_table(df=select_data, database=combo_login.get(), selected_DBtable='Summary: C & D', extra=df.reindex(idx))
                ...
                if BM_Not_same_cnt == 0 and CD_Not_same_cnt == 0:
                    if df_B_mode['BeamStyleIndex'] == 0:
                        df_B_mode.insert(1, 'bsIndexTrace', [20])
                        print(df_B_mode)
                    ...
                    if df_B_mode['BeamStyleIndex'] == 0:
                        df_B_mode.insert(1, 'bsIndexTrace', 15)
                    elif df_B_mode['BeamStyleIndex'] == 1:
                        df_B_mode.insert(1, 'bsIndexTrace', 20)
...
def main():
    try:
        ...
        btn_gen = Button(root_main, width=30, height=3, text='MeasSetGeneration', command=measset_gen(server_address, ID, password, database))
